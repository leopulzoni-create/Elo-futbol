...
def _partidos_oficiales_jugador(conn, jugador_id: int) -> int:
    ...
    cur.execute("SELECT COUNT(*) AS c FROM historial_elo WHERE jugador_id = ?", (jugador_id,))
    row = cur.fetchone()
...
def panel_resultados():
    ...
    if opciones:
        ...
        if "Empate" in resultado and dif_goles != 0:
            st.warning("Para empate, la diferencia de goles debe ser 0. Se ajustará automáticamente al registrar.")
        elif "Empate" not in resultado and dif_goles == 0:
            st.warning("Si hay ganador, la diferencia de goles debería ser > 0.")
        ...
        if st.button("✅ Registrar resultado", key="btn_registrar_resultado"):
            try:
                ...
                # Si oficial, actualizar ELO + historial_elo (con reparto por jugador y boost early-games)
                if oficial == "Oficial":
                    ...
                    for j in team1 + team2:
                        elo_pre = j["elo"]
                        delta = delta1_por_jugador if j in team1 else delta2_por_jugador
