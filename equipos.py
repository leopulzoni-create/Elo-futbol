...
# -------------------------
# Admin: edición rápida del roster (agregar / quitar jugadores desde esta pantalla)
# -------------------------
CUPO_PARTIDO = 10

def obtener_jugadores_activos():
    """Lista de jugadores activos para completar el roster desde Generar equipos."""
    conn = get_connection()
    cur = conn.cursor()
    cur.execute("""
        SELECT id, nombre
...
def construir_bloques(jugadores):
    ...
    Normaliza pj.bloque para evitar errores (espacios, "0", ints, etc).
    """
    ...
    for j in jugadores:
        ...
        # FIX: si viene int (1,2,3), lo convertimos a str antes de strip
        b_str = str(b).strip()
        if b_str == "" or b_str == "0":
            singles.append(j)
            continue
...
# -------------------------
# Rachas de camisetas (admin stats style, últimas 8 semanas, racha actual)
# -------------------------
def calcular_rachas_camiseta(partido_id: int, fecha_ref):
    ...
    fecha_ini = (fecha_ref - timedelta(days=60)).date()
...
def panel_generacion():
    ...
    if confirmado:
        ...
        if avisos:
            st.markdown("#### 📊 Rachas de camiseta (últimos 2 meses · racha actual)")
            for it in avisos:
                label = "Clara ⚪" if it["camiseta"] == "clara" else "Oscura ⬛"
                st.write(f"- 👕 {it['nombre']}: {label} · {it['veces']} partidos")
    ...
    # Lista con botón quitar (evita la vista duplicada)
    names = [j["nombre"] for j in jugadores]
    ids_asignados = {j["jugador_id"] for j in jugadores}

    c1, c2 = st.columns(2)
    with c1:
        st.markdown(f"**Inscriptos:** {len(names)}/{CUPO_PARTIDO}")
    with c2:
        if len(names) == CUPO_PARTIDO:
    ...
    for i, j in enumerate(jugadores):
        ...
        with cols[i % 2]:
            ...
            if st.button("Quitar", key=f"eq_quitar_{partido_id}_{j['jugador_id']}_{i}"):
                ...
                for k in ["_equipos_opciones", "_equipos_diffs", "_equipos_actual"]:
                    st.session_state.pop(k, None)
                st.rerun()

    # Completar roster desde acá (admin)
    faltan = max(0, CUPO_PARTIDO - len(names))
    if faltan > 0:
        st.divider()
    ...
    # Para generar equipos, el roster debe estar completo
    if len(names) != CUPO_PARTIDO:
        st.stop()

    # Mantengo tu sistema de duplas/tríos tal cual
